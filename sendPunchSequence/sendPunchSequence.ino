 ...
 * 2) the boolean leds[][] array is for tinkerCAD only. use the real CRGB leds[][] matrix instead
 * 3) Uncomment the print button functions. SendPunchSequence() should be called when
 * someone presses the Print button
 * 4) when generating the punch pattern and comparing the led values to 0 or 1, you may need to replace the "== 0" with "== CRGB(0x000000)"
 * 5) Remove the dummy kirby image
 * 6) Choose a printer algorithm depending on which one looks better (and remove the unused one):
 *  generateShadedPunchPattern() or generatePunchPattern()
 * 
*/
